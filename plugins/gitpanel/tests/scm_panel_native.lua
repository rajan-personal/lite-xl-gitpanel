...
check(#panel.list.rows==2 and panel.list.rows[1].key=="changes" and drawn("No unstaged changes"),"empty Staged/Conflicts hidden; Changes has intelligible empty state")
check(panel.list:count_box(panel.list.rows[1]).label=="0","empty Changes retains neutral zero count")
model.status=git.status("## main\0M  staged\0UU conflict\0"); draw()
check(panel.list.collapsed.staged and panel.list.collapsed.conflicts and #panel.list.rows==4,"hidden then returning groups retain collapse state")
